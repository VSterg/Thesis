...
ROI_acts(active_regions,:) = ROI_acts(active_regions,:)/dipoles_per_region; %get mean activity per region
ROI_acts(inactive_regions,:) = ROI_acts(inactive_regions,:)/5; %get mean activity per region
...
ICA_eyeblink_proj = mean_dev(single_comp_data(:,act_start:act_end)') ;
...
strips_local_proj = mean_dev(eyeblink_noise_strips(:,act_start-40:act_start+40)');
strips_local_proj = strips_local_proj/abs(max(strips_local_proj));
...
eyeblink_dev = 0.3;
data_frontal = EEG_noisy(33,:); %any frontal electrode will do
...
window = 40;      %%%%%
%%%%%%%%%%%%%%%%%%%%%%%
%window = 40 should work well with this simulated data. It should work well
%with real EEG data recorded at 256Hz. You can change it manually to be
%approximately the number of samples between the base and top of a spike
%caused by an eyeblink.
...
reference_idx = find(mean_ch_dist_norm==1);
eyeblink_noise_reference = eyeblink_noise_strips(reference_idx,:);
...
%%savefile then repeat with different seed
